...
import { ConflictError, HttpError, InternalServerError } from "@/api/http-error";
...
export const errorHandler = (err: Error, _req: Request, res: Response, _next: NextFunction) => {
  ...
  if (err instanceof HttpError) return res.error(err);

  res.error(new InternalServerError());
};
